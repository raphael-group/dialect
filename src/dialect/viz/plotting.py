...
def draw_average_simulation_precision_recall_curve(
    ...
    for idx, method_name in enumerate(methods):
        ...
        auprc_vals = []
        for y_true, methods_dict in zip(all_y_true, all_method_scores, strict=False):
...
def draw_auprc_hit_curve(
    ...
    for _, (true_pairs, top_ranked_table) in enumerate(
        zip(true_pair_lists, top_ranked_tables, strict=False),
    ):
...
def draw_cbase_likely_passenger_proportion_barplot(
    ...
    subtypes, proportions = zip(*sorted_subtype_to_proportion, strict=False)
    plt.barh(
        subtypes,
        proportions,
        color="black",
...
def draw_gene_expected_and_observed_mutations_barplot(
    ...
    figwidth = num_genes
    _fig, ax = plt.subplots(figsize=(figwidth, font_scale * 3))
...
def draw_mutation_variability_scatterplot(
    ...
    subtypes, coef_of_variations = zip(*sorted_subtypes, strict=False)
    plt.scatter(
        x=subtypes,
