...
def import_vcf_data(filename):
    ...
    if [line[:6] for line in lines].count("#CHROM") == 0:
        sys.stderr.write(
            "No vcf input file detected --> checking for samples in column 6.\n"
        )
        for line in lines[1:]:
            field = line.strip().split("\t")
            if N_columns >= 6:
            ...
            if field[0] == "X":
                ...
                chrnr = 24
            elif field[0] == "M" or field[0] == "MT":
                continue
            elif field[0].isdigit():
                chrnr = int(field[0])
            else:
                continue
            if bases.count(field[3]) and bases.count(field[4]):
                mut_array.append(
                    {
                        "chr": chrnr,
                        "pos": int(field[1]) - 1,
