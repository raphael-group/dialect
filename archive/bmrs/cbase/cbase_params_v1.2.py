...
def import_vcf_data(filename):
	...
	lines = fin.readlines()
	fin.close()
	lines = [line for line in lines if (line[:1]!='#' and line[:3]!='CHR') or line[:6]=='#CHROM']

	#	Note: Split by any white space, not just tabs.
	N_columns = len(lines[0].strip().split())

	if N_columns<5:
		sys.stderr.write("Number of columns in vcf file not as expected (>=5): %i.\n" %len(field))
		sys.stderr.write("Expected format: [1. CHROM (integer or X or Y), 2. POS (integer, 1-based), 3. ID (not used), 4. REF (string), 5. ALT (string), 6. SAMPLE (optional)]\n")
		sys.exit()
	...
	for line in lines:
		...
		if bases.count(field[3]) and bases.count(field[4]) and field[3]!=field[4]:
			mut_array.append({"chr": chrnr, "pos": int(field[1])-1, "ref": field[3].upper(), "alt": field[4].upper(), "sample": sample_name})	# annotation file coordinates are 0-based
	return mut_array
...
def import_effects_by_gene(filename):
	...
	fin.close()
	genes = []
	gene_target = [[[0 for eff in range(3)] for alt in range(4)] for cont in range(len(used_plets))]
	gene_info = {}

	cnt = 0
	for line in lines:
		sys.stderr.write("%i%% of importing effects by gene done.\r" %(100.*cnt/len(lines)))
		cnt += 1
		proxy = line.strip().split('\t')
		if len(proxy)>4:
			genes.append([gene_info, gene_target])
			gene_info = {"chr": int(proxy[0]), "gene": proxy[1], "transcript": proxy[2], "strand": proxy[3], "genebegin": int(proxy[4]), "geneend": int(proxy[5])}
			gene_target = [[[0 for eff in range(3)] for alt in range(4)] for cont in range(len(used_plets))]
		else:
