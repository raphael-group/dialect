...
def calculate_log_odds(tau):
    tau = (
        np.array(tau) + 1e-6
    )  # Add a small constant to avoid division by zero in the log calculation
    log_odds = np.log(
        (tau[1] * tau[2]) / (tau[0] * tau[3])
    )  # Calculate log odds using the provided formula
    return log_odds
