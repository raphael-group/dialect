...
#     while True:
#         ris = [
#             np.random.choice(range(ws_num), cur_num) for i in range(tsize)
#         ]  # choose random indices
#         cur_tuples = zip(*ris)
#         rtuples.extend(
#             filter(lambda ri: len(ri) == len(set(ri)), cur_tuples)
#         )  # no redundant indices in each tuple
...
def convert_maf_to_mut_list(maf_fn, dout):
    ...
    cnt_mtx = df.pivot_table(
        index="Hugo_Symbol", columns="Tumor_Sample_Barcode", aggfunc="size", fill_value=0
    ).T
