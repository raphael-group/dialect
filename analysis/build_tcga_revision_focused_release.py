...
def _readme(commit: str) -> Member:
    content = (
        ...
        "This hash-manifested candidate accompanies the corrected analysis of 10,433 "
        "participant-unique tumors in 32 TCGA Pan-Cancer Atlas cohorts. Each cohort "
        "uses the same 500 mutation events, samples, and tested pairs under CBaSE, "
        "DIG, and a sample-specific MutSig2CV-derived background. Same-gene "
        "missense:nonsense pairs are excluded.\n\n"
        "MutSig is the primary background for mutual exclusivity and co-occurrence; "
        "CBaSE is retained for mutual-exclusivity continuity and DIG as supplementary "
        "sensitivity. Provider overlap is descriptive, not an inferential vote. The "
        "frozen reporting rule uses one nondirectional profile likelihood-ratio test, "
        "complete-family Benjamini-Hochberg adjustment within cohort and provider, "
        "q <= 0.10 as primary, and q <= 0.20 as sensitivity. Direction is assigned "
        "from fitted rho only after testing.\n\n"
        f"Source commit: `{commit}` in `raphael-group/dialect`.\n\n"
        "## Contents\n\n"
        "- `documents/`: clean and marked manuscripts, Supporting Information, and "
        "the point-by-point response.\n"
        "- `results/postprocess/`: complete provider-specific derived association "
        "tables for every cohort.\n"
        "- `results/calibration/`: prespecified marginal and complete-null family "
        "calibration outputs.\n"
        "- `results/report/`: Table S5, figure and table source data, provider "
        "overlap, top primary pairs, runtime, and fit-certificate summaries.\n"
        "- `provenance/`: the source boundary and all 96 task receipts.\n\n"
        ...
        "Run from the source commit in the documented `dialect` environment. Replace "
        "angle-bracketed paths with local source roots.\n\n"
        "```bash\n"
        "python -m analysis.prepare_tcga_revision_focused inputs \\\n"
        "  --raw-maf-root <raw-maf-root> --population-root <population-root> \\\n"
        "  --datahub-git-dir <datahub-git-dir> --output-root <input-root>\n"
        "python -m analysis.prepare_tcga_revision_focused providers \\\n"
        "  --input-root <input-root> --provider-root <provider-root> --nice 10\n"
        "python -m analysis.run_tcga_revision_focused \\\n"
        "  --provider-root <provider-root> --output-root <run-root> \\\n"
        "  --jobs 3 --nice 10\n"
        "python -m analysis.postprocess_tcga_revision_focused \\\n"
        "  --run-root <run-root> --output-root <postprocess-root>\n"
        "python -m analysis.calibrate_tcga_revision_focused \\\n"
        "  --run-root <run-root> --provider-root <provider-root> \\\n"
        "  --output-root <calibration-root> --jobs 3 --nice 10\n"
        "python -m analysis.calibrate_tcga_revision_focused \\\n"
        "  --output-root <calibration-root> --summarize\n"
        "```\n\n"
        "## Integrity verification\n\n"
        "Verify the external receipt and every archive member without extraction:\n\n"
        "```bash\n"
        "python -m analysis.build_tcga_revision_focused_release \\\n"
        "  --verify-archive <release.tar.gz> --receipt <release.receipt.json>\n"
        "```\n"
    ).encode()
