...
POSTPROCESS_RELEASE_CONTRACT: Final = (
    "sealed-whole-grid-no-replace-publication-v1"
)
...
def _validate_cohort_policy(
    ...
    *,
    ...
    if (
        ...
        or manifest.get("valid_component_statuses")
        != list(VALID_COMPONENT_STATUSES)
        or conjunction.get("valid_component_statuses")
        != list(VALID_COMPONENT_STATUSES)
        or conjunction.get("component_order") != list(BMRS)
        or conjunction.get("mode") != "nondirectional-max-p-iut"
        or conjunction.get("p_value_combiner")
        != "max(p_cbase,p_dig,p_mutsig)"
    ...
    for provider in BMRS:
        ...
        if (
            set(component) != {"pairwise_sha256", "raw_schema"}
            or component.get("raw_schema") != list(RAW_PAIRWISE_COLUMNS)
        ):
            msg = (
                "Postprocess raw component schema is invalid for "
                f"{cohort}/{provider}."
            )
            raise SourceDataBuildError(msg)
        _require_sha256(
    ...
    if (
        ...
        or cohort_marginal.get("evidence_id")
        != root_marginal.get("evidence_id")
        or cohort_marginal.get("artifact_sha256")
        != root_marginal.get("sha256")
    ):
...
def _validate_cohort_production_authority(  # noqa: PLR0913
    ...
    *,
    ...
    if (
        root_contract.get("contract_sha256")
        != axis.get("cohort_contract_sha256")
    ...
    for provider, raw in zip(BMRS, bindings, strict=True):
        ...
        if (
            ...
            != axis.get("cohort_contract_sha256")
            or binding.get("pairwise_sha256")
            != component.get("pairwise_sha256")
        ):
...
def _validate_numeric_and_boolean_fields(
    ...
    *,
    ...
    for field_name in numeric:
        ...
        if not value:
            msg = (
                f"Missing production probability {field_name} "
                f"in {cohort} row {index}."
            )
            raise SourceDataBuildError(msg)
        try:
            parsed = float(value)
...
def _adjust_q_values(p_values: list[float], *, method: str) -> list[float]:
    ...
    count = len(ordered)
    factor = (
        math.fsum(1.0 / rank for rank in range(1, count + 1))
        if method == "by"
        else 1.0
    )
    adjusted = [
...
def _open_frozen_directory(
    ...
    *,
    ...
    if (
        ...
        os.close(descriptor)
        msg = f"{label} must be a frozen mode-0500 directory."
        raise SourceDataBuildError(msg)
    path_identity = resolved.stat(follow_symlinks=False)
    if (
        path_identity.st_dev != identity.st_dev
        or path_identity.st_ino != identity.st_ino
    ):
        os.close(descriptor)
...
def _revalidate_input_tree(
    ...
    path_root = root_path.stat(follow_symlinks=False)
    if (
        root_path.resolve(strict=True) != root_path
        or current_root.st_dev != root_identity.st_dev
        or current_root.st_ino != root_identity.st_ino
        or path_root.st_dev != root_identity.st_dev
        or path_root.st_ino != root_identity.st_ino
        or stat.S_IMODE(current_root.st_mode) != 0o500
    ):
        msg = "Postprocess release root changed during source-data publication."
        raise SourceDataBuildError(msg)
    _require_inventory(
        root_fd,
        {
    ...
    for pin in root_files:
        ...
        if (
            _hash_stable_descriptor(pin.descriptor, pin.identity, label=pin.name)
            != pin.sha256
        ):
    ...
    for pin in cohort_pins:
        ...
        if (
            identity.st_dev != pin.identity.st_dev
            or identity.st_ino != pin.identity.st_ino
            or stat.S_IMODE(identity.st_mode) != 0o500
        ):
            msg = f"Pinned postprocess cohort changed: {pin.cohort}."
            raise SourceDataBuildError(msg)
        _require_inventory(
            pin.descriptor,
            {POSTPROCESS_CSV_NAME, POSTPROCESS_COHORT_MANIFEST_NAME},
...
def _require_staged_output_identity(
    ...
    *,
    ...
    if (
        ...
        or _hash_stable_descriptor(descriptor, identity, label=name)
        != expected_sha256
    ):
...
def _validate_output_pin(
    ...
    *,
    ...
    if (
        ...
        or _hash_stable_descriptor(pin.descriptor, identity, label=pin.name)
        != pin.sha256
    ):
...
def main() -> None:
    """Run the closed production source-data publication entrypoint."""
